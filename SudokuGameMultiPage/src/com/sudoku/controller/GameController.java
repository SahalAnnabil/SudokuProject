            ...
            for (int c = 0; c < SudokuBoard.SIZE; c++) {
                ...
                cellView.addKeyListener(new KeyAdapter() {
                    ...
                    public void keyReleased(KeyEvent e) {
                        ...
                        int oldValue = currentBoard.getCell(row, col).getValue();
                        int newValue = cellView.getIntValue();

                        if (oldValue != newValue) {
                            if (currentBoard.setCellValue(row, col, newValue)) {
                                moveHistory.push(new MoveRecord(row, col, oldValue, newValue));
                                // Optional: live validation atau status update
                                boardGamePage.setStatusMessage("Cell (" + (row + 1) + "," + (col + 1) + ") = " + (newValue == 0 ? " " : newValue), false);
                            } else { // Jika setCellValue gagal (misal karena fixed, seharusnya sudah dicek)
                                cellView.setValueInView(oldValue, false); // Kembalikan ke nilai lama di view
                            }
                        }
                    }
                });
